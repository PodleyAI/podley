...
if (existingOutput) {
  ...
  // Add new job
  const newJobId = await queue.add(new DownloadJob({ 
...
import { getSqliteDatabase } from "@podley/sqlite";

const db = getSqliteDatabase("./jobs.db");
const queue = new JobQueue("my-queue", MyJob, {
  storage: new SqliteQueueStorage(db, "my-queue"),
...
const limiter = new CompositeLimiter([
  ...
  new InMemoryRateLimiter({     // Max 20 per minute
    maxExecutions: 20,
    windowSizeInSeconds: 60
  }),
]);
...
interface IJobQueue<Input, Output> {
  ...
  // Progress tracking
  updateProgress(jobId: unknown, progress: number, message?: string, details?: Record<string, any>): Promise<void>;
