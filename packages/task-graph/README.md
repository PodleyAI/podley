...
A powerful TypeScript library for building and executing DAG (Directed Acyclic Graph) pipelines of tasks. Provides flexible task orchestration, persistent storage, workflow management, and comprehensive error handling for complex task execution scenarios.
...
class MultiplyTask extends Task<{ value: number }, { result: number }> {
  static readonly type = "MultiplyTask";
...
// 2. Using TaskGraph (low-level API)
const graph = new TaskGraph();
graph.addTask(new MultiplyTask({ value: 5 }, { id: "multiply1" }));
graph.addTask(new MultiplyTask({ value: 10 }, { id: "multiply2" }));

const results = await graph.run();
console.log(results); // [{ id: "multiply1", data: { result: 10 } }, { id: "multiply2", data: { result: 20 } }]

// 3. Using Workflow (high-level API)
const workflow = new Workflow();
workflow.addTask(new MultiplyTask({ value: 15 }));
...
class TextProcessorTask extends Task<MyInput, MyOutput> {
  ...
  // Optional: Lightweight execution for UI updates
  async executeReactive(input: MyInput, output: MyOutput): Promise<MyOutput> {
    // Use cached output if available, or compute quickly
    return output || { processed: input.text, length: input.text.length };
  }
}
```
...
const doubleTask = new MultiplyTask({ value: 5 }, { id: "double" });
const tripleTask = new MultiplyTask({ value: 5 }, { id: "triple" });
const addTask = new AddTask({}, { id: "add" });

const graph = new TaskGraph();
graph.addTasks([doubleTask, tripleTask, addTask]);

// Connect outputs to inputs
graph.addDataflow(new Dataflow("double", "result", "add", "a"));
graph.addDataflow(new Dataflow("triple", "result", "add", "b"));

const results = await graph.run();
// Results: double=10, triple=10, add=20
```
...
workflow.addTask(new MultiplyTask({ value: 5 }));  // Outputs: { result: number }
workflow.addTask(new MultiplyTask({}));             // Inputs: { value: number }
workflow.addTask(new MultiplyTask({}));             // Inputs: { value: number }
...
const workflow = new Workflow();
workflow
  .MyTextProcessor({ text: "Hello" })
  .MyTextProcessor({ multiplier: 3 });
...
// Tasks with same inputs will use cached outputs
const task1 = new MultiplyTask({ value: 5 });
const result1 = await task1.run(); // Executes and caches

const task2 = new MultiplyTask({ value: 5 });  
const result2 = await task2.run(); // Uses cache
```
...
// Save task graph
const graph = new TaskGraph();
graph.addTask(new MultiplyTask({ value: 10 }));
...
class CompositeTask extends GraphAsTask<{ input: string }, { output: string }> {
  ...
  constructor(input: { input: string }, config: any = {}) {
    ...
    this.subGraph.addDataflow(new Dataflow(
      subTask1.config.id, "processed",
      subTask2.config.id, "text"
    ));
  }
}
```
...
class TaskFactory extends Task<{ count: number }, { results: any[] }> {
  async execute(input: { count: number }, context: IExecuteContext) {
    ...
    for (let i = 0; i < input.count; i++) {
      ...
      const dynamicTask = new MultiplyTask({ value: i });
