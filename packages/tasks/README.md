...
A comprehensive package for building and running DAG pipelines with various task types for common operations, workflow management, and data processing. This package provides building blocks for creating complex task graphs with support for HTTP requests, JavaScript execution, delays, logging, and dynamic task creation.
...
- [Available Tasks](#available-tasks)
  - [FetchTask](#fetchtask) - HTTP requests with retry logic
  - [DebugLogTask](#debuglogtask) - Console logging and debugging
  - [DelayTask](#delaytask) - Timed delays with progress tracking
  - [JavaScriptTask](#javascripttask) - Execute JavaScript code strings
  - [LambdaTask](#lambdatask) - Execute custom functions
  - [JsonTask](#jsontask) - Build workflows from JSON definitions
...
// Error logging
await DebugLog({
  console: "Failed to process request",
  log_level: "error"
});
...
for (let i = 0; i < 5; i++) {
  await Fetch({ url: `https://api.example.com/item/${i}` });
  await Delay({ delay: 1000 }); // 1 second between requests
}
```
...
// Using input data
const processed = await JavaScript({
  code: `
...
// String processing
const textResult = await JavaScript({
  code: `
...
const reactiveResult = await Lambda(
  { message: "Hello" },
  {
...
const transformer = await Lambda(
...
const apiProcessor = await Lambda(
  { userId: 123 },
  {
    execute: async (input, context) => {
...
interface JsonTaskItem {
  ...
  dependencies?: {         // Input dependencies from other tasks
    [inputField: string]: {
...
const linearWorkflow = await Json({
  json: JSON.stringify([
    {
      id: "fetch-data",
      type: "FetchTask",
...
const complexWorkflow = await Json({
  json: JSON.stringify([
    {
      id: "fetch-users",
      type: "FetchTask",
...
const dynamicWorkflow = await Json({
